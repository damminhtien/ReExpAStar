...
class WeightedAStarCR(Generic[State]):
    ...
    def __init__(self, start: State, is_goal: GoalTest[State], neighbors: NeighborFn[State], h: HeuristicFn[State],
        ...
        self.start = start; self.is_goal = is_goal; self.neighbors = neighbors; self.h = h
        self.w = float(weight); self.r = float(r); self.r_mode = r_mode
        ...
        self.open.push(start, f0, kb); self.status[start] = 'OPEN'
    ...
    def _reopen_allowed(self, delta: float, edge_cost: float, old_g: float) -> bool:
        if self.r_mode == 'abs': return delta > self.r
        if self.r_mode == 'rel_edge': return delta > self.r * edge_cost
        if self.r_mode == 'rel_g': return delta > self.r * max(1.0, old_g if math.isfinite(old_g) else 1.0)
        return delta > self.r
    def search_until_goal(self) -> Tuple[Optional[List[State]], Optional[float]]:
        ...
        while True:
            ...
            if popped is None:
                ...
                return None, None
            s, _, _ = popped
            if s not in self.g: continue
            self.status[s] = 'CLOSED'; self.stats.expansions += 1
            if self.log_every and (self.stats.expansions % self.log_every == 0):
                self.logger.info(f'expansions={self.stats.expansions}, generated={self.stats.generated}, reopens={self.stats.reopens}')
            ...
            for t, cost in self.neighbors(s):
                ...
                if self.status.get(t) == 'OPEN':
                    ...
                    if self._reopen_allowed(delta, float(cost), old_g):
                        ...
                        kb = new_g if self.tie_break=='g_low' else -new_g
                        self.open.push(t, new_g + self.w * self._h(t), kb)
                        self.status[t] = 'OPEN'; self.stats.reopens += 1
                    else:
                        self.repairs.append(ProposedUpdate(delta=delta, state=t, new_g=new_g, parent=s))
                else:
                    self.g[t] = new_g; self.parent[t] = s; self.status[t] = 'OPEN'
                    kb = new_g if self.tie_break=='g_low' else -new_g
                    self.open.push(t, new_g + self.w * self._h(t), kb)
    def lower_r_and_repair(self, new_r: float) -> int:
        assert new_r <= self.r
        self.r = float(new_r)
        if not self.repairs: return 0
        self.repairs.sort(key=lambda u: u.delta, reverse=True)
        accepted = 0; remaining: List[ProposedUpdate] = []
        for upd in self.repairs:
            ...
            if upd.new_g < cur_g and self._reopen_allowed(delta, 1.0, cur_g):
                self.g[upd.state] = upd.new_g; self.parent[upd.state] = upd.parent
                kb = self.g[upd.state] if self.tie_break=='g_low' else -self.g[upd.state]
                self.open.push(upd.state, self._f(upd.state), kb)
                self.status[upd.state] = 'OPEN'; accepted += 1
            else:
                new_delta = max(0.0, delta)
                if new_delta > 0.0:
                    remaining.append(ProposedUpdate(delta=new_delta, state=upd.state, new_g=upd.new_g, parent=upd.parent))
        self.repairs = remaining; return accepted
    def run_to_first_goal(self) -> Tuple[Optional[List[State]], Optional[float], SearchStats]:
        path, cost = self.search_until_goal(); return path, cost, self.stats
    def run_ira_schedule(self, r_values: List[float]) -> Tuple[List[Tuple[float, float]], Optional[List[State]]]:
        history: List[Tuple[float, float]] = []; best_cost = math.inf; best_path: Optional[List[State]] = None
        p, c = self.search_until_goal()
        if p is not None and c is not None: best_cost = c; best_path = p
        history.append((self.r, best_cost))
        for new_r in r_values[1:]:
            self.lower_r_and_repair(new_r); p, c = self.search_until_goal()
            if p is not None and c is not None and c < best_cost: best_cost = c; best_path = p
            history.append((self.r, best_cost))
        return history, best_path

@dataclass
class Grid:
    width: int; height: int; walls: set[Tuple[int, int]] = field(default_factory=set); step: float = 1.0
    ...
    def neighbors4(self, p: Tuple[int, int]) -> Iterable[Tuple[Tuple[int, int], float]]:
        ...
        for dx, dy in [(1,0), (-1,0), (0,1), (0,-1)]:
            q = (x+dx, y+dy)
            if self.in_bounds(q) and self.passable(q): yield q, self.step
    ...
    def manhattan(self, goal: Tuple[int, int]) -> HeuristicFn:
        def h(p: Tuple[int, int]) -> float: return abs(p[0]-goal[0]) + abs(p[1]-goal[1])
        return h
...
def compare_on_grid(width: int = 50, height: int = 50, weight: float = 1.5, r_cr: float = 0.2):
    grid = Grid(width, height, walls={(width//2, y) for y in range(height)} - {(width//2, height//3)})
    start, goal = (0, 0), (width-1, height-1)
    h = grid.manhattan(goal); is_goal = lambda s: s == goal
    ...
    return [
        run_engine('NR', WeightedAStarNR(start, is_goal, grid.neighbors, h, weight=weight)),
        run_engine('AR', WeightedAStarAR(start, is_goal, grid.neighbors, h, weight=weight)),
        run_engine(f'CR(r={r_cr})', WeightedAStarCR(start, is_goal, grid.neighbors, h, weight=weight, r=r_cr)),
    ]
