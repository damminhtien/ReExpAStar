...
def main():
    ...
    gap_y = args.height // 3; walls.remove((args.width//2, gap_y))
    ...
    def emit_row(name, eng):
        ...
        row = dict(algo=name, w=eng.w, r=getattr(eng, 'r', None), r_mode=getattr(eng, 'r_mode', None),
                   cost=cost, expansions=st.expansions, reopens=st.reopens, generated=st.generated,
                   runtime_ms=st.runtime_ms, path_len=(len(path) if path else None))
        return row

    rows = []
    rows.append(emit_row('NR', WeightedAStarNR(start, is_goal, grid.neighbors, h, weight=args.weight)))
    rows.append(emit_row('AR', WeightedAStarAR(start, is_goal, grid.neighbors, h, weight=args.weight)))
    for r in r_list:
        if math.isinf(r):
            continue
        rows.append(emit_row(f'CR(r={r})', WeightedAStarCR(start, is_goal, grid.neighbors, h, weight=args.weight, r=r, r_mode=args.r_mode)))

    keys = ['algo','w','r','r_mode','cost','expansions','reopens','generated','runtime_ms','path_len']
    print(','.join(keys))
    for row in rows:
