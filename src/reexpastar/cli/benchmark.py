...
from reexpastar.scenarios import (
    scenario_grid_4, scenario_grid_8, scenario_grid_inconsistent,
    scenario_geometric, scenario_puzzle, scenario_terrain8, scenario_maze4
)
from reexpastar.ara_star import ARAStar

def run_one(name: str, eng):
    path, cost, st = eng.run_to_first_goal()
    return dict(cost=cost, expansions=st.expansions, reopens=getattr(st, 'reopens', 0),
                generated=st.generated, runtime_ms=st.runtime_ms, path_len=(len(path) if path else None))
...
def main():
    ...
    for tok in args.r_values.split(','):
        t = tok.strip().lower()
        if t in ('inf','infinity','∞'): r_list.append(math.inf)
        else: r_list.append(float(t))

    keys = ['scenario','kind','algo','w','r','r_mode','cost','expansions','reopens','generated','runtime_ms','path_len']
    lines = [','.join(keys)]
    for sc in scenarios:
        if args.include_ara:
            ...
            final_cost = hist[-1][1]
            lines.append(','.join(map(str,[sc.name, sc.meta['kind'], 'ARA*', args.weight, None, None, final_cost, ara.stats.expansions, 0, ara.stats.generated, None, (len(best_path) if best_path else None)])))
        for r in r_list:
            if math.isinf(r):
                engines = [('NR', WeightedAStarNR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=args.weight))]
            else:
                engines = [
                    ('AR', WeightedAStarAR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=args.weight)),
                    (f'CR(r={r})', WeightedAStarCR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=args.weight, r=r, r_mode=args.r_mode)),
                ]
            for name, eng in engines:
                row = run_one(name, eng)
                out = dict(
                    scenario=sc.name, kind=sc.meta['kind'], algo=name, w=args.weight,
    ...
    out_path = args.out or os.path.join("results", f"benchmark_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
