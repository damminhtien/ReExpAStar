...
def run_job(job: Tuple[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if algo == "NR":
        ...
        r = cfg["r"]
        eng = WeightedAStarCR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=weight, r=r, r_mode=r_mode)
    path, cost, st = eng.run_to_first_goal()
    return dict(scenario=sc.name, kind=sc.meta['kind'], algo=algo, w=weight, r=(cfg.get("r")), r_mode=r_mode,
                cost=cost, expansions=st.expansions, reopens=st.reopens, generated=st.generated, runtime_ms=st.runtime_ms,
                path_len=(len(path) if path else None), seed=cfg["scenario_kwargs"].get("seed", None))
...
def main():
    ...
    for (ctor, base_kwargs), seed in itertools.product(scenario_ctors, seeds):
        kwargs = dict(base_kwargs); kwargs["seed"] = seed
        for algo in ["NR", "AR"]:
            jobs.append((f"{ctor.__name__}-{algo}-seed{seed}", dict(scenario_ctor=ctor, scenario_kwargs=kwargs, algo=algo, weight=args.weight, r_mode=args.r_mode)))
        for r in r_list:
            jobs.append((f"{ctor.__name__}-CR{r}-seed{seed}", dict(scenario_ctor=ctor, scenario_kwargs=kwargs, algo="CR", weight=args.weight, r=r, r_mode=args.r_mode)))
    ...
    out_path = os.path.join("results", f"benchmark_parallel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    keys = ['scenario','kind','algo','w','r','r_mode','cost','expansions','reopens','generated','runtime_ms','path_len','seed']
