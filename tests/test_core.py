...
class TestWeightedAStarCR(unittest.TestCase):
    def setUp(self):
        ...
        self.h = self.grid.manhattan(self.goal); self.is_goal = lambda s: s == self.goal
    def test_extremes_equivalence_consistent_h(self):
        engine_nr = WeightedAStarCR(self.start, self.is_goal, self.grid.neighbors, self.h, weight=1.0, r=math.inf)
        _, c_nr, _ = engine_nr.run_to_first_goal()
        engine_ar = WeightedAStarCR(self.start, self.is_goal, self.grid.neighbors, self.h, weight=1.0, r=0.0)
        _, c_ar, _ = engine_ar.run_to_first_goal()
        self.assertAlmostEqual(c_nr, c_ar); self.assertEqual(c_nr, 18.0)
    def test_weighted_bound_empirical(self):
        w = 1.5
        base = WeightedAStarCR(self.start, self.is_goal, self.grid.neighbors, self.h, weight=1.0, r=0.0)
        _, c_opt, _ = base.run_to_first_goal()
        for r in (0.0, math.inf):
            eng = WeightedAStarCR(self.start, self.is_goal, self.grid.neighbors, self.h, weight=w, r=r)
    ...
    def test_no_overwrite_without_improvement(self):
        ...
        def neigh(u: str):
            for v, c in graph.get(u, []): yield v, c
        def h(u: str)->float: return 0.0
        is_goal = lambda u: u == 'A'
        eng = WeightedAStarCR('S', is_goal, neigh, h, weight=1.0, r=0.0)
        _, c, _ = eng.run_to_first_goal()
        self.assertEqual(c, 10.0); self.assertEqual(eng.parent['A'], 'S')
...
class TestDiverseScenarios(unittest.TestCase):
    ...
    def test_inconsistent_heuristic_triggers_reopens(self):
        ...
        nr = WeightedAStarNR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=1.5)
        _, _, st_nr = nr.run_to_first_goal()
        ar = WeightedAStarAR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=1.5)
        _, _, st_ar = ar.run_to_first_goal()
        self.assertEqual(st_nr.reopens, 0); self.assertGreaterEqual(st_ar.reopens, 1)
    def test_geometric_graph_solution(self):
        ...
        eng = WeightedAStarCR(sc.start, sc.is_goal, sc.neighbors, sc.h, weight=1.2, r=0.2)
        p, c, _ = eng.run_to_first_goal()
        self.assertIsNotNone(p); self.assertIsNotNone(c); self.assertGreater(len(p), 1)
...
class TestTerrainAndMaze(unittest.TestCase):
    def test_terrain_admissible_heuristic(self):
        ...
        eng = WeightedAStarCR(start, lambda s: s==goal, tg.neighbors8, h, weight=1.0, r=0.0)
        _, c, _ = eng.run_to_first_goal()
        self.assertIsNotNone(c); self.assertGreaterEqual(c, h(start) - 1e-9)
    def test_maze_reachability(self):
        ...
        eng = WeightedAStarCR(start, lambda s: s==goal, g.neighbors4, g.manhattan(goal), weight=1.0, r=0.0)
        p, c, _ = eng.run_to_first_goal()
        self.assertIsNotNone(c); self.assertGreater(len(p), 1)
